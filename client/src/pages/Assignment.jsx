...
const Assignment = () => {
  ...
  return (
    <div className="w-screen h-screen items-center justify-center flex bg-rzt_bg overflow-hidden">
      <div className="bg-white p-5 sm:px-5 down-md:h-[95%] max-h-[95%]  rounded-lg shadow max-w-[1279px] w-full flex flex-col gap-3 overflow-y-scroll scrollbar-hide ">
        <button
          onClick={handleClickHome}
          className="w-full  border-b-4 border-rzt_blue flex sm:justify-center mb-2"
        >
          <img
            src={ImgLogo}
            alt="Logo Rozetle"
        ...
        <p>
          {assign?.assignment?.senderId?.name +
            ...
            assign?.assignment?.senderId?.email[0]}{" "}
          sana <b> {assign?.assignment?.badgeId?.title} </b>
          rozetini gönderdi
        </p>
        <p className="text-xs text-gray-600">
          Rozeti Hesabına eklemek için 
        </p>
        <div className="flex flex-wrap justify-evenly gap-4">
          ...
          >
            <img
              src={ImgPlaystore}
              alt="play-store"
              className="max-w-[200px] sm:max-w-[150px]"
            />
          </button>
          <button
            onClick={() => {
